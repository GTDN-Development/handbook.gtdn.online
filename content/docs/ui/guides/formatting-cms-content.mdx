...
description: "Style HTML content from CMS systems with the prose utility class"
highlight: true
---

import { Step, Steps } from "fumadocs-ui/components/steps";

When rendering HTML content from a CMS or remote markdown, the content lacks styling since it arrives as raw semantic HTML. The `.prose` utility class provides consistent typography styling for this content. The prose styles automatically also adapt to dark mode.


## Usage

Wrap CMS or remote HTML content with a container that has the `prose` class:
...
<Step>
**Add the prose styles**

```css title="src/styles/prose.css"
.prose {
  --prose-color: color-mix(in oklab, var(--foreground) 80%, transparent);
  ...
  --prose-marker-color: color-mix(in oklab, var(--foreground) 25%, transparent);
  --prose-link-underline-color: var(--primary);
  --prose-th-borders: var(--border);
  --prose-td-borders: var(--border);
  --prose-hr-color: color-mix(in oklab, var(--foreground) 5%, transparent);
  ...
  &:where(.dark, .dark *) {
    ...
    --prose-link-color: var(--primary);
    --prose-code-color: var(--foreground);
    --prose-marker-color: color-mix(in oklab, var(--foreground) 35%, transparent);
    --prose-link-underline-color: var(--primary);
    --prose-th-borders: var(--border);
    --prose-td-borders: var(--border);
    --prose-hr-color: color-mix(in oklab, var(--foreground) 10%, transparent);
    --prose-blockquote-border-color: var(--border);
  }
...
/* Rest of your globals.css config */
```
</Step>
</Steps>
...
The `.prose-blog` class extends prose with additional blog-specific styling:

```tsx
<Container size="sm" className="prose prose-blog">
  <div dangerouslySetInnerHTML={{ __html: blogContent }} />
</Container>
```

## Opting Out with not-prose

Elements with the `not-prose` class and their descendants are excluded from prose styling. Use this for custom components, embeds, or any content that needs its own styling:

```tsx
<div className="prose">
  <h2>Styled heading</h2>
  <p>Styled paragraph with prose typography.</p>
  ...
  <p>Back to styled prose content.</p>
</div>
```
