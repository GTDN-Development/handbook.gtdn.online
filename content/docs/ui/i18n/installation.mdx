...
<Steps>
 ...
 import { defineRouting } from "next-intl/routing";
  ...
  export const { Link, getPathname, redirect, usePathname, useRouter } = createNavigation(routing);
   ```
     It defines supported languages (en, cs, others..).
 ...
 <Step>
  ...
  <Step>
   ...
   ```tsx title="src/i18n/request.ts"
    ...
    ```
    It renders translations based on the locale.
  </Step>
  <Step>
     **Create the middleware file**
 
     ```bash
     touch src/i18n/middleware.ts
     ```
   </Step>
  <Step>
    **Copy & paste the source code**
...
<Step>
    **Create the Providers File**
    ...
    ```bash
    touch src/components/layout/providers.tsx
    ```
  </Step>
  <Step>
    **Global Providers**
  ...
  <Step>
    **Create the layout root**
    ...
    ```bash
    touch src/components/layout/layout-root.tsx
    ```
  </Step>
  <Step>
    **Copy & paste the source code**
  ...
  export default async function LayoutRoot({
    ...
    return (
      ...
      >
        <body
          className="scroll-pt-16 overflow-x-clip scroll-smooth font-sans antialiased"
          suppressHydrationWarning
        >
...
This component holds `<html>`, fonts, and `NextIntlClientProvider` .
  ...
  <Step>
     **Create the sitemap file**
     ...
     ```bash
     touchsrc/app/sitemap.ts
     ```
   </Step>
  <Step>
    **Copy & paste the source code**

     ```tsx title="src/app/sitemap.tsx"
...
Every page has to call `setRequestLocale` for static site generation. The structure of the page should be as follows:
...
<Step>
**Update your messages.json files**

The structure of the messages.json files should be as follows:
